...
def client_handler(client, address):
    ...
    try:
        while True:
            ...
            logger.info("%s:%d" % (address[0], address[1]) + " CMD Received  --> " + response + " (" + str(len(response)) + " char)")
            if not STATO[0] == "MOVING":
                ...
                if response.split()[0] == "best" and len(response.split()) > 1:
                    ...
                    if len(response) > 0:
                        ...
                        if response.split()[0] == "best":
                            logger.info("%s:%d" % (address[0], address[1]) + " Asking BEST-2 declination to the Antenna... -> "+response)
                            ...
                            logger.warning("%s:%d" % (address[0], address[1]) + " Unknown command received! -> "+response)
                            message = "UNKNOWN CMD --> " + response.split()[0]
                    else:
                        logger.info("%s:%d" % (address[0], address[1])+" Connection closed with client %s:%d" % (address[0], address[1]))
                        break
                ...
                if len(response.split()) == 1:
                    ...
                    if response.split()[0] == "best":
                        message = "MOVING "
                        message += str(POSIZIONI[NORD1])
                        logger.info("%s:%d" % (address[0], address[1]) + " Reporting BEST-2 declination (Antenna is moving) --> " + message)
                    elif response.split()[0] == "status":
                        message = "MOVING "
                        message += report_status()
                        logger.info("%s:%d" % (address[0], address[1]) + " Reporting Antenna status (while moving) --> " + message)

                    else:
                        logger.warning("%s:%d" % (address[0], address[1]) + " Unknown command received or received a command not supported while Antenna is moving! (" + response.split()[0] + ")")
                        message = "CMD unknown or not supported while antenna is moving (" + response.split()[0] + ")"
                elif len(response.split()) == 2:
                    logger.warning("%s:%d" % (address[0], address[1]) + " Unsupported command received while moving --> " + response)
